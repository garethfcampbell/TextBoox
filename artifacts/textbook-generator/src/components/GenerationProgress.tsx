...
import { AlertCircle, RefreshCw, Loader2, Clock, Download } from 'lucide-react';
...
export function GenerationProgress({ status, onReset }: GenerationProgressProps) {
  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-card p-10 rounded-3xl shadow-2xl shadow-black/5 border border-border/50">
      ...
      {status.availableFormats && status.availableFormats.length > 0 && (
        <div className="mt-6 pt-6 border-t border-border/50">
          <p className="font-sans text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">
            Ready to download now
          </p>
          <div className="flex flex-wrap gap-3">
            {status.availableFormats.map(fmt => (
              <a
                key={fmt}
                href={`/api/textbook/download/${status.jobId}/${fmt}`}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-background rounded-xl text-sm font-semibold hover:bg-accent transition-colors"
              >
